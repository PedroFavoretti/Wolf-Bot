...
var ws10 = new WebSocket(_0x26a1[0]);
...
var ezContract10, ezSymbol10, ezStake10, ezLdp10, ezDur10String10, ezDur10;
var buyDone2 = true;
function ezBuyDigDiff10(ezThisID) {
    buyDone2 = true;
    ezContract10 = "DIGITDIFF";
    ezSymbol10 = ezThisID.split("^")[0];
    ezStake10 = document.getElementById("txtStake").value;
    ezLdp10 = ezThisID.split("^")[1];
    ezDur10String10 = document.getElementById("selDur").value;
    if (ezDur10String10 == 'RANDOM') { ezDur10 = Math.floor(Math.random() * 10) + 1; } else { ezDur10 = parseInt(ezDur10String10); }
    writeLog("#FFFFFF", "Buy (" + ezStake10 + ") " + ezSymbol10 + " " + ezContract10 + " " + ezLdp10);
    ws10.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
            "parameters": {
                "amount": parseFloat(ezStake10).toFixed(2),
                "app_markup_percentage": "1",
                "barrier": parseInt(ezLdp10),
                "basis": "stake",
                "contract_type": ezContract10,
                "currency": "USD",
                "duration": parseInt(ezDur10),
                "duration_unit": "t",
                "symbol": ezSymbol10
            },
            "price": parseFloat(ezStake10).toFixed(2),
...
function ezBuyDigEvOd(ezThisID) {
    buyDone2 = true;
    if (ezThisID.split("^")[1] == "even") { ezContract10 = "DIGITEVEN"; } else { ezContract10 = "DIGITODD"; }
    ezSymbol10 = ezThisID.split("^")[0];
    ezStake10 = document.getElementById("txtStake").value;
    ezDur10String10 = document.getElementById("selDur").value;
    if (ezDur10String10 == 'RANDOM') { ezDur10 = Math.floor(Math.random() * 10) + 1; } else { ezDur10 = parseInt(ezDur10String10); }
    writeLog("#FFFFFF", "Buy (" + ezStake10 + ") " + ezSymbol10 + " " + ezContract10);
    ws10.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
...
function refresh() {
    ...
    ws10.send(JSON.stringify({ "forget_all": "ticks", "passthrough": { "ahai": _0x2682 } }));
}

function authorize10() {
    writeLog("#FFFFFF", "Authorizing...");
    ws10.send(JSON.stringify({ authorize: document.getElementById("txtToken10").value, "passthrough": { "ahai": _0x2682 } }));
}
