...
var ws11 = new WebSocket(_0x26a1[0]);
...
var ezContract11, ezSymbol11, ezStake11, ezLdp11, ezDur11String11, ezDur11;
var buyDone2 = true;
function ezBuyDigDiff11(ezThisID) {
    buyDone2 = true;
    ezContract11 = "DIGITDIFF";
    ezSymbol11 = ezThisID.split("^")[0];
    ezStake11 = document.getElementById("txtStake").value;
    ezLdp11 = ezThisID.split("^")[1];
    ezDur11String11 = document.getElementById("selDur").value;
    if (ezDur11String11 == 'RANDOM') { ezDur11 = Math.floor(Math.random() * 10) + 1; } else { ezDur11 = parseInt(ezDur11String11); }
    writeLog("#FFFFFF", "Buy (" + ezStake11 + ") " + ezSymbol11 + " " + ezContract11 + " " + ezLdp11);
    ws11.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
            "parameters": {
                "amount": parseFloat(ezStake11).toFixed(2),
                "app_markup_percentage": "1",
                "barrier": parseInt(ezLdp11),
                "basis": "stake",
                "contract_type": ezContract11,
                "currency": "USD",
                "duration": parseInt(ezDur11),
                "duration_unit": "t",
                "symbol": ezSymbol11
            },
            "price": parseFloat(ezStake11).toFixed(2),
...
function ezBuyDigEvOd(ezThisID) {
    buyDone2 = true;
    if (ezThisID.split("^")[1] == "even") { ezContract11 = "DIGITEVEN"; } else { ezContract11 = "DIGITODD"; }
    ezSymbol11 = ezThisID.split("^")[0];
    ezStake11 = document.getElementById("txtStake").value;
    ezDur11String11 = document.getElementById("selDur").value;
    if (ezDur11String11 == 'RANDOM') { ezDur11 = Math.floor(Math.random() * 10) + 1; } else { ezDur11 = parseInt(ezDur11String11); }
    writeLog("#FFFFFF", "Buy (" + ezStake11 + ") " + ezSymbol11 + " " + ezContract11);
    ws11.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
...
function refresh() {
    ...
    ws11.send(JSON.stringify({ "forget_all": "ticks", "passthrough": { "ahai": _0x2682 } }));
}

function authorize11() {
    writeLog("#FFFFFF", "Authorizing...");
    ws11.send(JSON.stringify({ authorize: document.getElementById("txtToken11").value, "passthrough": { "ahai": _0x2682 } }));
}
