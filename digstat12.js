...
var ws12 = new WebSocket(_0x26a1[0]);
...
var ezContract12, ezSymbol12, ezStake12, ezLdp12, ezDur12String12, ezDur12;
var buyDone2 = true;
function ezBuyDigDiff12(ezThisID) {
    buyDone2 = true;
    ezContract12 = "DIGITDIFF";
    ezSymbol12 = ezThisID.split("^")[0];
    ezStake12 = document.getElementById("txtStake").value;
    ezLdp12 = ezThisID.split("^")[1];
    ezDur12String12 = document.getElementById("selDur").value;
    if (ezDur12String12 == 'RANDOM') { ezDur12 = Math.floor(Math.random() * 10) + 1; } else { ezDur12 = parseInt(ezDur12String12); }
    writeLog("#FFFFFF", "Buy (" + ezStake12 + ") " + ezSymbol12 + " " + ezContract12 + " " + ezLdp12);
    ws12.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
            "parameters": {
                "amount": parseFloat(ezStake12).toFixed(2),
                "app_markup_percentage": "1",
                "barrier": parseInt(ezLdp12),
                "basis": "stake",
                "contract_type": ezContract12,
                "currency": "USD",
                "duration": parseInt(ezDur12),
                "duration_unit": "t",
                "symbol": ezSymbol12
            },
            "price": parseFloat(ezStake12).toFixed(2),
...
function ezBuyDigEvOd(ezThisID) {
    buyDone2 = true;
    if (ezThisID.split("^")[1] == "even") { ezContract12 = "DIGITEVEN"; } else { ezContract12 = "DIGITODD"; }
    ezSymbol12 = ezThisID.split("^")[0];
    ezStake12 = document.getElementById("txtStake").value;
    ezDur12String12 = document.getElementById("selDur").value;
    if (ezDur12String12 == 'RANDOM') { ezDur12 = Math.floor(Math.random() * 10) + 1; } else { ezDur12 = parseInt(ezDur12String12); }
    writeLog("#FFFFFF", "Buy (" + ezStake12 + ") " + ezSymbol12 + " " + ezContract12);
    ws12.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
...
function refresh() {
    ...
    ws12.send(JSON.stringify({ "forget_all": "ticks", "passthrough": { "ahai": _0x2682 } }));
}

function authorize12() {
    writeLog("#FFFFFF", "Authorizing...");
    ws12.send(JSON.stringify({ authorize: document.getElementById("txtToken12").value, "passthrough": { "ahai": _0x2682 } }));
}
