...
var ws13 = new WebSocket(_0x26a1[0]);
...
var ezContract13, ezSymbol13, ezStake13, ezLdp13, ezDur13String13, ezDur13;
var buyDone2 = true;
function ezBuyDigDiff13(ezThisID) {
    buyDone2 = true;
    ezContract13 = "DIGITDIFF";
    ezSymbol13 = ezThisID.split("^")[0];
    ezStake13 = document.getElementById("txtStake").value;
    ezLdp13 = ezThisID.split("^")[1];
    ezDur13String13 = document.getElementById("selDur").value;
    if (ezDur13String13 == 'RANDOM') { ezDur13 = Math.floor(Math.random() * 10) + 1; } else { ezDur13 = parseInt(ezDur13String13); }
    writeLog("#FFFFFF", "Buy (" + ezStake13 + ") " + ezSymbol13 + " " + ezContract13 + " " + ezLdp13);
    ws13.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
            "parameters": {
                "amount": parseFloat(ezStake13).toFixed(2),
                "app_markup_percentage": "1",
                "barrier": parseInt(ezLdp13),
                "basis": "stake",
                "contract_type": ezContract13,
                "currency": "USD",
                "duration": parseInt(ezDur13),
                "duration_unit": "t",
                "symbol": ezSymbol13
            },
            "price": parseFloat(ezStake13).toFixed(2),
...
function ezBuyDigEvOd(ezThisID) {
    buyDone2 = true;
    if (ezThisID.split("^")[1] == "even") { ezContract13 = "DIGITEVEN"; } else { ezContract13 = "DIGITODD"; }
    ezSymbol13 = ezThisID.split("^")[0];
    ezStake13 = document.getElementById("txtStake").value;
    ezDur13String13 = document.getElementById("selDur").value;
    if (ezDur13String13 == 'RANDOM') { ezDur13 = Math.floor(Math.random() * 10) + 1; } else { ezDur13 = parseInt(ezDur13String13); }
    writeLog("#FFFFFF", "Buy (" + ezStake13 + ") " + ezSymbol13 + " " + ezContract13);
    ws13.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
...
function refresh() {
    ...
    ws13.send(JSON.stringify({ "forget_all": "ticks", "passthrough": { "ahai": _0x2682 } }));
}

function authorize13() {
    writeLog("#FFFFFF", "Authorizing...");
    ws13.send(JSON.stringify({ authorize: document.getElementById("txtToken13").value, "passthrough": { "ahai": _0x2682 } }));
}
