...
var ws14 = new WebSocket(_0x26a1[0]);
...
var ezContract14, ezSymbol14, ezStake14, ezLdp14, ezDur14String14, ezDur14;
var buyDone2 = true;
function ezBuyDigDiff14(ezThisID) {
    buyDone2 = true;
    ezContract14 = "DIGITDIFF";
    ezSymbol14 = ezThisID.split("^")[0];
    ezStake14 = document.getElementById("txtStake").value;
    ezLdp14 = ezThisID.split("^")[1];
    ezDur14String14 = document.getElementById("selDur").value;
    if (ezDur14String14 == 'RANDOM') { ezDur14 = Math.floor(Math.random() * 10) + 1; } else { ezDur14 = parseInt(ezDur14String14); }
    writeLog("#FFFFFF", "Buy (" + ezStake14 + ") " + ezSymbol14 + " " + ezContract14 + " " + ezLdp14);
    ws14.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
            "parameters": {
                "amount": parseFloat(ezStake14).toFixed(2),
                "app_markup_percentage": "1",
                "barrier": parseInt(ezLdp14),
                "basis": "stake",
                "contract_type": ezContract14,
                "currency": "USD",
                "duration": parseInt(ezDur14),
                "duration_unit": "t",
                "symbol": ezSymbol14
            },
            "price": parseFloat(ezStake14).toFixed(2),
...
function ezBuyDigEvOd(ezThisID) {
    buyDone2 = true;
    if (ezThisID.split("^")[1] == "even") { ezContract14 = "DIGITEVEN"; } else { ezContract14 = "DIGITODD"; }
    ezSymbol14 = ezThisID.split("^")[0];
    ezStake14 = document.getElementById("txtStake").value;
    ezDur14String14 = document.getElementById("selDur").value;
    if (ezDur14String14 == 'RANDOM') { ezDur14 = Math.floor(Math.random() * 10) + 1; } else { ezDur14 = parseInt(ezDur14String14); }
    writeLog("#FFFFFF", "Buy (" + ezStake14 + ") " + ezSymbol14 + " " + ezContract14);
    ws14.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
...
function refresh() {
    ...
    ws14.send(JSON.stringify({ "forget_all": "ticks", "passthrough": { "ahai": _0x2682 } }));
}

function authorize14() {
    writeLog("#FFFFFF", "Authorizing...");
    ws14.send(JSON.stringify({ authorize: document.getElementById("txtToken14").value, "passthrough": { "ahai": _0x2682 } }));
}
