...
var ws15 = new WebSocket(_0x26a1[0]);
...
var ezContract15, ezSymbol15, ezStake15, ezLdp15, ezDur15String15, ezDur15;
var buyDone2 = true;
function ezBuyDigDiff15(ezThisID) {
    buyDone2 = true;
    ezContract15 = "DIGITDIFF";
    ezSymbol15 = ezThisID.split("^")[0];
    ezStake15 = document.getElementById("txtStake").value;
    ezLdp15 = ezThisID.split("^")[1];
    ezDur15String15 = document.getElementById("selDur").value;
    if (ezDur15String15 == 'RANDOM') { ezDur15 = Math.floor(Math.random() * 10) + 1; } else { ezDur15 = parseInt(ezDur15String15); }
    writeLog("#FFFFFF", "Buy (" + ezStake15 + ") " + ezSymbol15 + " " + ezContract15 + " " + ezLdp15);
    ws15.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
            "parameters": {
                "amount": parseFloat(ezStake15).toFixed(2),
                "app_markup_percentage": "1",
                "barrier": parseInt(ezLdp15),
                "basis": "stake",
                "contract_type": ezContract15,
                "currency": "USD",
                "duration": parseInt(ezDur15),
                "duration_unit": "t",
                "symbol": ezSymbol15
            },
            "price": parseFloat(ezStake15).toFixed(2),
...
function ezBuyDigEvOd(ezThisID) {
    buyDone2 = true;
    if (ezThisID.split("^")[1] == "even") { ezContract15 = "DIGITEVEN"; } else { ezContract15 = "DIGITODD"; }
    ezSymbol15 = ezThisID.split("^")[0];
    ezStake15 = document.getElementById("txtStake").value;
    ezDur15String15 = document.getElementById("selDur").value;
    if (ezDur15String15 == 'RANDOM') { ezDur15 = Math.floor(Math.random() * 10) + 1; } else { ezDur15 = parseInt(ezDur15String15); }
    writeLog("#FFFFFF", "Buy (" + ezStake15 + ") " + ezSymbol15 + " " + ezContract15);
    ws15.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
...
function refresh() {
    ...
    ws15.send(JSON.stringify({ "forget_all": "ticks", "passthrough": { "ahai": _0x2682 } }));
}

function authorize15() {
    writeLog("#FFFFFF", "Authorizing...");
    ws15.send(JSON.stringify({ authorize: document.getElementById("txtToken15").value, "passthrough": { "ahai": _0x2682 } }));
}
