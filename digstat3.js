...
var ws3 = new WebSocket(_0x26a1[0]);
...
var ezContract3, ezSymbol3, ezStake3, ezLdp3, ezDur3String3, ezDur3;
var buyDone2 = true;
function ezBuyDigDiff3(ezThisID) {
    buyDone2 = true;
    ezContract3 = "DIGITDIFF";
    ezSymbol3 = ezThisID.split("^")[0];
    ezStake3 = document.getElementById("txtStake").value;
    ezLdp3 = ezThisID.split("^")[1];
    ezDur3String3 = document.getElementById("selDur").value;
    if (ezDur3String3 == 'RANDOM') { ezDur3 = Math.floor(Math.random() * 10) + 1; } else { ezDur3 = parseInt(ezDur3String3); }
    writeLog("#FFFFFF", "Buy (" + ezStake3 + ") " + ezSymbol3 + " " + ezContract3 + " " + ezLdp3);
    ws3.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
            "parameters": {
                "amount": parseFloat(ezStake3).toFixed(2),
                "app_markup_percentage": "1",
                "barrier": parseInt(ezLdp3),
                "basis": "stake",
                "contract_type": ezContract3,
                "currency": "USD",
                "duration": parseInt(ezDur3),
                "duration_unit": "t",
                "symbol": ezSymbol3
            },
            "price": parseFloat(ezStake3).toFixed(2),
...
function ezBuyDigEvOd(ezThisID) {
    buyDone2 = true;
    if (ezThisID.split("^")[1] == "even") { ezContract3 = "DIGITEVEN"; } else { ezContract3 = "DIGITODD"; }
    ezSymbol3 = ezThisID.split("^")[0];
    ezStake3 = document.getElementById("txtStake").value;
    ezDur3String3 = document.getElementById("selDur").value;
    if (ezDur3String3 == 'RANDOM') { ezDur3 = Math.floor(Math.random() * 10) + 1; } else { ezDur3 = parseInt(ezDur3String3); }
    writeLog("#FFFFFF", "Buy (" + ezStake3 + ") " + ezSymbol3 + " " + ezContract3);
    ws3.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
...
function refresh() {
    ...
    ws3.send(JSON.stringify({ "forget_all": "ticks", "passthrough": { "ahai": _0x2682 } }));
}

function authorize3() {
    writeLog("#FFFFFF", "Authorizing...");
    ws3.send(JSON.stringify({ authorize: document.getElementById("txtToken3").value, "passthrough": { "ahai": _0x2682 } }));
}
