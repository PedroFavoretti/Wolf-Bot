...
var ws8 = new WebSocket(_0x26a1[0]);
...
var ezContract8, ezSymbol8, ezStake8, ezLdp8, ezDur8String8, ezDur8;
var buyDone2 = true;
function ezBuyDigDiff8(ezThisID) {
    buyDone2 = true;
    ezContract8 = "DIGITDIFF";
    ezSymbol8 = ezThisID.split("^")[0];
    ezStake8 = document.getElementById("txtStake").value;
    ezLdp8 = ezThisID.split("^")[1];
    ezDur8String8 = document.getElementById("selDur").value;
    if (ezDur8String8 == 'RANDOM') { ezDur8 = Math.floor(Math.random() * 10) + 1; } else { ezDur8 = parseInt(ezDur8String8); }
    writeLog("#FFFFFF", "Buy (" + ezStake8 + ") " + ezSymbol8 + " " + ezContract8 + " " + ezLdp8);
    ws8.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
            "parameters": {
                "amount": parseFloat(ezStake8).toFixed(2),
                "app_markup_percentage": "1",
                "barrier": parseInt(ezLdp8),
                "basis": "stake",
                "contract_type": ezContract8,
                "currency": "USD",
                "duration": parseInt(ezDur8),
                "duration_unit": "t",
                "symbol": ezSymbol8
            },
            "price": parseFloat(ezStake8).toFixed(2),
...
function ezBuyDigEvOd(ezThisID) {
    buyDone2 = true;
    if (ezThisID.split("^")[1] == "even") { ezContract8 = "DIGITEVEN"; } else { ezContract8 = "DIGITODD"; }
    ezSymbol8 = ezThisID.split("^")[0];
    ezStake8 = document.getElementById("txtStake").value;
    ezDur8String8 = document.getElementById("selDur").value;
    if (ezDur8String8 == 'RANDOM') { ezDur8 = Math.floor(Math.random() * 10) + 1; } else { ezDur8 = parseInt(ezDur8String8); }
    writeLog("#FFFFFF", "Buy (" + ezStake8 + ") " + ezSymbol8 + " " + ezContract8);
    ws8.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
...
function refresh() {
    ...
    ws8.send(JSON.stringify({ "forget_all": "ticks", "passthrough": { "ahai": _0x2682 } }));
}

function authorize8() {
    writeLog("#FFFFFF", "Authorizing...");
    ws8.send(JSON.stringify({ authorize: document.getElementById("txtToken8").value, "passthrough": { "ahai": _0x2682 } }));
}
