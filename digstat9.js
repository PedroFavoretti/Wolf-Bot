...
var ws9 = new WebSocket(_0x26a1[0]);
...
var ezContract9, ezSymbol9, ezStake9, ezLdp9, ezDur9String9, ezDur9;
var buyDone2 = true;
function ezBuyDigDiff9(ezThisID) {
    buyDone2 = true;
    ezContract9 = "DIGITDIFF";
    ezSymbol9 = ezThisID.split("^")[0];
    ezStake9 = document.getElementById("txtStake").value;
    ezLdp9 = ezThisID.split("^")[1];
    ezDur9String9 = document.getElementById("selDur").value;
    if (ezDur9String9 == 'RANDOM') { ezDur9 = Math.floor(Math.random() * 10) + 1; } else { ezDur9 = parseInt(ezDur9String9); }
    writeLog("#FFFFFF", "Buy (" + ezStake9 + ") " + ezSymbol9 + " " + ezContract9 + " " + ezLdp9);
    ws9.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
            "parameters": {
                "amount": parseFloat(ezStake9).toFixed(2),
                "app_markup_percentage": "1",
                "barrier": parseInt(ezLdp9),
                "basis": "stake",
                "contract_type": ezContract9,
                "currency": "USD",
                "duration": parseInt(ezDur9),
                "duration_unit": "t",
                "symbol": ezSymbol9
            },
            "price": parseFloat(ezStake9).toFixed(2),
...
function ezBuyDigEvOd(ezThisID) {
    buyDone2 = true;
    if (ezThisID.split("^")[1] == "even") { ezContract9 = "DIGITEVEN"; } else { ezContract9 = "DIGITODD"; }
    ezSymbol9 = ezThisID.split("^")[0];
    ezStake9 = document.getElementById("txtStake").value;
    ezDur9String9 = document.getElementById("selDur").value;
    if (ezDur9String9 == 'RANDOM') { ezDur9 = Math.floor(Math.random() * 10) + 1; } else { ezDur9 = parseInt(ezDur9String9); }
    writeLog("#FFFFFF", "Buy (" + ezStake9 + ") " + ezSymbol9 + " " + ezContract9);
    ws9.send(JSON.stringify(
        {
            "subscribe": 1,
            "buy": 1,
...
function refresh() {
    ...
    ws9.send(JSON.stringify({ "forget_all": "ticks", "passthrough": { "ahai": _0x2682 } }));
}

function authorize9() {
    writeLog("#FFFFFF", "Authorizing...");
    ws9.send(JSON.stringify({ authorize: document.getElementById("txtToken9").value, "passthrough": { "ahai": _0x2682 } }));
}
